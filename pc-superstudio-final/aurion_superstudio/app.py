...
def protect_mutations():
    if request.method not in {"GET", "HEAD", "OPTIONS"} and request.headers.get("X-Aurion-Token") != TOKEN:
...
def run() -> None:
    app.run(host="127.0.0.1", port=int(os.environ.get("AURION_STUDIO_PORT", "5060")), debug=False, threaded=True)
