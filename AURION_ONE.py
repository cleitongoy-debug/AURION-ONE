...
BUILD="2026.09.23-T8I-RAW-LAB-15"
...
# ---------- CANON T8i · LAB RAW ADITIVO ----------
try:
    import aurion_t8i
    aurion_t8i.register_t8i(app, ROOT, log)
except Exception as _t8i_exc:
    log("T8I RAW: módulo não carregado: "+str(_t8i_exc)[:500], True)
...
<button onclick="tab('image',this)">GERAR IMAGEM</button><button onclick="tab('t8i',this);t8iStatus()">CANON T8i · RAW</button><button onclick="tab('comfy',this)">COMFYUI REAL</button><button onclick="tab('manager',this)">MANAGER / SCAN</button>
...
<section id="t8i" class="tab"><div class="card">
<h3>CANON EOS REBEL T8i · LAB RAW CR3</h3>
<p><b>Revelação local não destrutiva.</b> O arquivo original importado é copiado para <code>ORIGINAIS</code> e nunca é sobrescrito. A aba cria estrutura própria, registra SHA-256, parâmetros, saídas, conversas e snapshots.</p>
<p style="color:var(--warn)">Nota técnica: fotografia .CR3 é RAW de sensor; C-Log é curva de vídeo. Aqui o motor revela RAW estático da T8i/Canon sem fingir que CR3 é C-Log.</p>
<div class="grid2">
  <div class="card"><h3>1 · MOTOR / DEPENDÊNCIAS</h3><pre id="t8iDeps">Consultando...</pre><button onclick="t8iInstall()">INSTALAR / REPARAR MOTOR RAW</button><button onclick="t8iStatus()">ATUALIZAR STATUS</button></div>
  <div class="card"><h3>2 · DEPÓSITO DO PROJETO</h3><input id="t8iWorkspace" placeholder="Escolha onde criar AURION_T8I"><div class="row"><button onclick="t8iPickFolder()">ESCOLHER PASTA 🗂️</button><button onclick="t8iApplyFolder()">CRIAR / USAR PASTA</button><button onclick="t8iOpenWorkspace()">ABRIR PASTA</button></div><pre id="t8iFolders">ORIGINAIS · REVELADOS · EXPORTADOS · CONVERSAS · PRESETS · LOGS · BACKUPS</pre></div>
</div>
<div class="grid2">
  <div class="card"><h3>3 · IMPORTAR RAW DA T8i</h3><input id="t8iFile" type="file" accept=".cr3,.cr2,.dng"><button class="primary" onclick="t8iImport()">IMPORTAR SEM ALTERAR ORIGINAL</button><p id="t8iSource">Nenhum RAW importado nesta sessão.</p><small>O servidor cria uma cópia de trabalho e um manifesto com tamanho + SHA-256.</small></div>
  <div class="card"><h3>4 · REVELAR</h3><label>Exposição EV <input id="t8iExposure" type="range" min="-5" max="5" step="0.1" value="0" oninput="t8iExpValue.textContent=this.value"><b id="t8iExpValue">0</b></label><label>Contraste <input id="t8iContrast" type="range" min="0.25" max="3" step="0.05" value="1"></label><label>Saturação <input id="t8iSaturation" type="range" min="0" max="3" step="0.05" value="1"></label><label>Balanço de branco <select id="t8iWB"><option value="camera">Da câmera</option><option value="auto">Automático pelo RAW</option><option value="neutral">Neutro / sem WB automático</option></select></label><button class="primary" onclick="t8iDevelop()">REVELAR · TIFF 16-BIT + JPEG</button></div>
</div>
<div class="grid2">
  <div class="card"><h3>5 · PRÉVIA / SAÍDA</h3><div id="t8iPreview" class="result">A prévia aparecerá aqui.</div><pre id="t8iResult" style="white-space:pre-wrap;max-height:260px;overflow:auto"></pre></div>
  <div class="card"><h3>6 · ARQUIVOS DA SESSÃO</h3><button onclick="t8iFiles()">ATUALIZAR DEPÓSITOS</button><div id="t8iFileList"></div></div>
</div>
<div class="card"><h3>7 · CONVERSAS / NOTAS · NADA PERDIDO</h3><input id="t8iConvTitle" placeholder="Título da conversa / ensaio" oninput="t8iDraftSave()"><textarea id="t8iConvText" style="min-height:150px" placeholder="Cole aqui decisões, prompts, conversa com IA, ajustes, observações do ensaio..." oninput="t8iDraftSave()"></textarea><div class="row"><button class="primary" onclick="t8iSaveConversation()">SALVAR CONVERSA EM DISCO</button><button onclick="t8iSnapshot()">SALVAR SNAPSHOT DA ABA</button><button onclick="t8iDraftLoad()">RECUPERAR RASCUNHO LOCAL</button></div><pre id="t8iSaveStatus">Rascunho também é preservado no navegador enquanto você digita.</pre></div>
</div></section>
...
let T8I_SOURCE='', T8I_DRAFT_KEY='aurion_t8i_conversation_draft_v1', _t8iStatusTimer=null;
function t8iUiSettings(){return {source:T8I_SOURCE,exposure:Number(document.getElementById('t8iExposure')?.value||0),contrast:Number(document.getElementById('t8iContrast')?.value||1),saturation:Number(document.getElementById('t8iSaturation')?.value||1),white_balance:document.getElementById('t8iWB')?.value||'camera'}}
async function t8iStatus(){const box=document.getElementById('t8iDeps');if(!box)return;try{const r=await fetch('/api/one/t8i/status',{cache:'no-store'}),d=await r.json();document.getElementById('t8iWorkspace').value=d.workspace||'';document.getElementById('t8iFolders').textContent=JSON.stringify(d.folders||{},null,2);box.textContent=JSON.stringify({dependencies:d.dependencies,versions:d.versions,installer:d.installer,note:d.note},null,2);if(d.installer&&d.installer.running){clearTimeout(_t8iStatusTimer);_t8iStatusTimer=setTimeout(t8iStatus,2500)}}catch(e){box.textContent='Falha: '+e.message}}
async function t8iInstall(){document.getElementById('t8iDeps').textContent='Instalação solicitada no Python atual...';try{await fetch('/api/one/t8i/install',{method:'POST'});setTimeout(t8iStatus,1200)}catch(e){document.getElementById('t8iDeps').textContent='Falha: '+e.message}}
async function t8iPickFolder(){const s=document.getElementById('t8iSaveStatus');s.textContent='Abrindo seletor do Windows...';try{const r=await fetch('/api/one/t8i/pick-folder',{method:'POST'}),d=await r.json();if(!r.ok)throw Error(d.error||r.status);if(d.path){document.getElementById('t8iWorkspace').value=d.path;s.textContent='Pasta escolhida. Clique CRIAR / USAR PASTA.'}else s.textContent='Seleção cancelada.'}catch(e){s.textContent='Falha no seletor: '+e.message}}
async function t8iApplyFolder(){const s=document.getElementById('t8iSaveStatus'),path=document.getElementById('t8iWorkspace').value.trim();s.textContent='Criando estrutura sem apagar arquivos existentes...';try{const r=await fetch('/api/one/t8i/workspace',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({path})}),d=await r.json();if(!r.ok)throw Error(d.error||r.status);s.textContent='Estrutura pronta: '+d.workspace;await t8iStatus();await t8iFiles()}catch(e){s.textContent='Falha: '+e.message}}
async function t8iOpenWorkspace(){try{const r=await fetch('/api/one/t8i/open-workspace',{method:'POST'}),d=await r.json();if(!r.ok)throw Error(d.error||r.status)}catch(e){document.getElementById('t8iSaveStatus').textContent='Não abriu a pasta: '+e.message}}
async function t8iImport(){const inp=document.getElementById('t8iFile'),status=document.getElementById('t8iSource');if(!inp.files||!inp.files[0]){status.textContent='Selecione CR3, CR2 ou DNG.';return}const fd=new FormData();fd.append('file',inp.files[0]);status.textContent='Copiando RAW para ORIGINAIS e calculando SHA-256...';try{const r=await fetch('/api/one/t8i/import',{method:'POST',body:fd}),d=await r.json();if(!r.ok)throw Error(d.error||r.status);T8I_SOURCE=d.source;status.textContent=d.name+' · '+d.bytes+' bytes · SHA-256 '+d.sha256;await t8iFiles()}catch(e){status.textContent='Falha: '+e.message}}
async function t8iDevelop(){const out=document.getElementById('t8iResult'),preview=document.getElementById('t8iPreview');out.textContent='Revelando RAW localmente...';try{const payload=t8iUiSettings(),r=await fetch('/api/one/t8i/develop',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}),d=await r.json();if(!r.ok)throw Error(d.error||r.status);T8I_SOURCE=d.source;preview.innerHTML='<img alt="Prévia revelada Canon RAW" style="max-width:100%;max-height:62vh;border-radius:10px" src="'+d.preview_url+'?t='+Date.now()+'">';out.textContent=JSON.stringify(d.manifest,null,2);await t8iFiles()}catch(e){out.textContent='Falha real: '+e.message}}
async function t8iFiles(){const box=document.getElementById('t8iFileList');if(!box)return;try{const d=await(await fetch('/api/one/t8i/files',{cache:'no-store'})).json();const cards=(title,arr)=>'<h4>'+title+' · '+arr.length+'</h4>'+arr.slice(0,24).map(x=>'<div class="card"><b>'+esc(x.name)+'</b><br><small>'+esc(x.path)+' · '+x.bytes+' bytes</small></div>').join('');box.innerHTML=cards('ORIGINAIS',d.originals||[])+cards('REVELADOS',d.revealed||[])+cards('CONVERSAS',d.conversations||[])}catch(e){box.textContent='Falha: '+e.message}}
function t8iDraftSave(){try{localStorage.setItem(T8I_DRAFT_KEY,JSON.stringify({title:document.getElementById('t8iConvTitle').value,text:document.getElementById('t8iConvText').value,updated:new Date().toISOString()}));document.getElementById('t8iSaveStatus').textContent='Rascunho local atualizado '+new Date().toLocaleTimeString('pt-BR')}catch(e){}}
function t8iDraftLoad(){try{const d=JSON.parse(localStorage.getItem(T8I_DRAFT_KEY)||'{}');document.getElementById('t8iConvTitle').value=d.title||'';document.getElementById('t8iConvText').value=d.text||'';document.getElementById('t8iSaveStatus').textContent=d.updated?'Rascunho recuperado de '+new Date(d.updated).toLocaleString('pt-BR'):'Nenhum rascunho salvo.'}catch(e){document.getElementById('t8iSaveStatus').textContent='Rascunho ilegível.'}}
async function t8iSaveConversation(){const s=document.getElementById('t8iSaveStatus'),payload={title:document.getElementById('t8iConvTitle').value,text:document.getElementById('t8iConvText').value,metadata:t8iUiSettings()};s.textContent='Salvando cópia imutável em CONVERSAS...';try{const r=await fetch('/api/one/t8i/conversation',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}),d=await r.json();if(!r.ok)throw Error(d.error||r.status);s.textContent='Salvo: '+d.markdown+' + '+d.json;await t8iFiles()}catch(e){s.textContent='Falha: '+e.message}}
async function t8iSnapshot(){const s=document.getElementById('t8iSaveStatus');try{const r=await fetch('/api/one/t8i/session-snapshot',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({raw:t8iUiSettings(),conversation_title:document.getElementById('t8iConvTitle').value,draft_present:!!document.getElementById('t8iConvText').value})}),d=await r.json();if(!r.ok)throw Error(d.error||r.status);s.textContent='Snapshot salvo: '+d.snapshot}catch(e){s.textContent='Falha: '+e.message}}
